...
class Roll {
  ...
  int rollInRange(int minInclusive, int maxExclusive) =>
      minInclusive + _random.nextInt(maxExclusive - minInclusive);

  int rollMultipleInRange(
      int minInclusive, int maxExclusive, int numDice, int addToTotal) {
    int _rollTotal = 0;

    if (numDice < 0 || maxExclusive < minInclusive || addToTotal == null) {
      throw Exception(
          'Error: called rollMultipleInRange with incorrect parameters');
    }
